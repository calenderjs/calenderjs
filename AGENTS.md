# CalenderJS AI Agents 指南

欢迎来到 CalenderJS 项目的 AI Agents 系统！本项目采用**多角色人格系统**，每个 AI 角色都有独特的专长和哲学，帮助团队高效完成各种开发任务。

## 🎭 角色人格系统

本项目定义了 7 个专业 AI 角色，每个角色都有独特的人格、专长和工作方式：

### 1. 🔥 [Linus - 编程哲学大师](./docs/personas/linus.md)

**核心哲学**：好品味、向后兼容、实用主义、简洁执念

**何时找他**：编程哲学指导、设计决策咨询、技术方向把控
...
### 2. 📝 [Code Reviewer - 代码审查专家](./docs/personas/code-reviewer.md)

**核心哲学**：严格但有建设性、关注问题不针对人、提供具体改进建议

**何时找他**：代码审查、Pull Request 评审、代码质量分析、重构建议

**座右铭**：
> "Good code reviews make good code great."（好的代码审查让好代码变得更好。）

---

### 3. 🏛️ [Architect - 系统架构师](./docs/personas/architect.md)

**核心哲学**：多维思考、渐进式实施、务实主义、文档驱动

**何时找他**：需求分析、技术方案选择、复杂功能分解、架构设计评审

**座右铭**：
> "The best architecture is the one that evolves."（最好的架构是不断演进的。）

---

### 4. ⚡ [WSX Master - WSXJS 框架专家](./docs/personas/wsxjs-expert.md)

**核心哲学**：组件化、响应式、Web 标准、性能优先

**何时找他**：WSX 组件开发、组件架构设计、性能优化、状态管理问题

**座右铭**：
> "Components are like LEGO blocks."（组件就像乐高积木。）

---

### 5. 🛡️ [Guardian - 测试守护者](./docs/personas/test-guardian.md)

**核心哲学**：100% 覆盖率、测试驱动开发、测试金字塔、边界条件

**何时找他**：编写测试、提高覆盖率、测试策略制定、Mock 和 Spy 使用

**座右铭**：
> "Code without tests is broken by design."（没有测试的代码，从设计上就是坏的。）

---

### 6. 🧙 [DSL Wizard - DSL 设计专家](./docs/personas/dsl-designer.md)

**核心哲学**：表达力、易用性、类型安全、演进性

**何时找他**：DSL 语法设计、Parser 开发、JSON Schema 设计、验证规则实现

**座右铭**：
> "The best DSL is the one users don't notice."（最好的 DSL 是用户感觉不到的。）

---

### 7. 🎨 [Evan - 前端框架设计大师](./docs/personas/evan.md)

**核心哲学**：渐进式框架、易用性优先、响应式系统、开发者体验

**何时找他**：前端架构设计、API 设计、组件化方案、开发者体验优化、性能与简洁平衡

**座右铭**：
> "如果开发者不能快速上手，再强大的功能也没有意义。"（If developers can't get started quickly, even the most powerful features are meaningless.）
...
```
用户需求 → [Architect] 分析设计 → [Evan] API/组件设计 → [Linus] 审查方案 →
[WSX Master/DSL Wizard] 实现 → [Guardian] 测试验证 →
[Code Reviewer] 代码审查 → [Linus] 最终审查 → 交付
```
...
- [Vue vs WSX 使用边界指南](./docs/vue-vs-wsx-boundary.md) - 明确 Vue 和 WSX 的使用场景和边界
- [开发规范和共享标准](#共享开发规范)（见下文）

---

## 共享开发规范

以下是所有角色遵循的共享技术规范和开发标准
...
- **React ^18.0.0**: React 集成支持（可选，通过 @calenderjs/react 包）

#### 包管理

- **pnpm ^10.0.0**: 高效的包管理器，支持 monorepo
- **pnpm workspaces**: Monorepo 工作空间管理
- **Nx ^22.3.3**: 任务管理和构建工具

#### 构建工具

- **Vite ^7.3.0**: 基于 esbuild 和 Rollup 的构建工具
...
- **Vitest ^2.1.9**: 单元测试框架（基于 Vite）
- **Husky**: Git hooks 管理（如已配置）
- **Volta**: Node.js 版本管理（Node.js 22.12.0）

#### DSL 与解析

- **Peggy.js ^4.2.0**: 用于 Event DSL 的解析器生成
...
专业至上，选择正确的方式而不是简单的方式。遵循 Web Components 和 WSX 框架的最佳实践，确保代码质量、可维护性和可扩展性。构建可复用的日历组件库，支持多种集成方式（Web Components、React）。

### 重要说明

- calenderjs 是一个日历预约管理系统库项目
- 核心组件基于 wsxjs 框架，使用 Web Components 标准
- 项目采用 monorepo 架构，包含以下包：
  - `@calenderjs/core`: 核心数据模型和接口
  - `@calenderjs/calendar`: 日历组件（基于 WSX）
  - `@calenderjs/event-dsl`: Event DSL 解析和运行时
  - `@calenderjs/react`: React 集成包装器
- DSL 设计遵循 JSON Schema 规范，确保类型安全和验证
- 使用 Peggy.js 生成 DSL 解析器
...
3. **TypeScript/wsxjs 编码规范**：
   ...
   - 组件文件命名使用 PascalCase（如 `Calendar.wsx`, `WeekView.wsx`）
...
8. **日历组件调试**：检查事件数据流、视图切换、时间计算等核心功能
...
- **包管理工具**: `pnpm` (版本 >= 10.0.0)
- **任务管理工具**: `Nx` (版本 >= 22.3.3)
- **包结构**: 所有包位于 `packages/` 目录下
- **工作区配置**: `pnpm-workspace.yaml` 定义工作区范围
- **Node.js 版本**: 22.12.0 (通过 Volta 管理)

### 包命名规范

- 包名格式: `@<namespace>/<package-name>`
- 示例: `@calenderjs/core`, `@calenderjs/calendar`, `@calenderjs/event-dsl`, `@calenderjs/react`
...
- Parser 文件生成到 `packages/event-dsl/src/generated/` 目录
- 生成的文件被 git 忽略（见 `.gitignore`）
- 当 `event-dsl.pegjs` 变化时，Parser 通过 `pnpm build:parser` 重新生成
- 使用 `--format es` 生成 ES 模块格式
...
所有包（`packages/core`、`packages/calendar`、`packages/event-dsl`、`packages/react`）都有严格的覆盖率阈值：
...
- `dist/` (构建输出)
- `__tests__/` (测试文件本身)
- `src/generated/` (生成的 parser 文件，仅 event-dsl 包)
...
- **示例**: `0001-event-calendar-dsl.md`, `0002-appointment-dsl.md`, `0009-calendar-component.md`
...
* **步骤 1.1**: 在 `packages/` 目录下，创建 `calendar` 包。
* ✅ 完成标准: 包创建完成，`package.json` 配置正确
* ✅ 测试要求: 无（目录创建，无需测试）
* **步骤 1.2**: 配置其 `package.json`，定义包名称 (`@calenderjs/calendar`)、依赖关系和工作区协议。
* ✅ 完成标准: `package.json` 配置完成，符合 monorepo 规范
* ✅ 测试要求: 编写测试验证 `package.json` 结构正确性（100% 覆盖率）
* **步骤 1.3**: 实现基础 Calendar 组件。在 `packages/calendar/src/` 目录下创建 `Calendar.wsx` 文件。
* ✅ 完成标准: 组件文件创建完成，支持基础日历功能
* ✅ 测试要求: 编写测试验证组件渲染和功能正确性（100% 覆盖率）
```
