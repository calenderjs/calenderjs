...
describe("generateRecurringInstances", () => {
  ...
  describe("daily 重复", () => {
    ...
    it("应该生成 daily 重复实例（使用 endDate）", () => {
      ...
      expect(
        instances[instances.length - 1].startTime.getTime(),
      ).toBeLessThanOrEqual(new Date("2025-01-03T10:00:00Z").getTime());
    });
  ...
  describe("实例属性", () => {
    ...
    it("应该保持原始事件的持续时间", () => {
      ...
      const instances = generateRecurringInstances(baseEvent, recurringRule);
      instances.forEach((inst) => {
        const duration = inst.endTime.getTime() - inst.startTime.getTime();
