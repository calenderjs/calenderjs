...
`;
      const ast = parseEventDSL(dsl);
      expect(ast.recurring).toBeDefined();
      expect(ast.recurring?.excludeDates).toEqual(["2025-01-01", "2025-12-25"]);
...
`;
      ...
      expect(ast.recurring?.daysOfWeek).toEqual([1, 3, 5]);
      expect(ast.recurring?.endDate).toBe("2025-12-31");
      expect(ast.recurring?.excludeDates).toEqual(["2025-01-01", "2025-07-04"]);
...
validate:
  tags in [["work", "urgent"], ["personal", "low"]]`;
      ...
      expect(Array.isArray((ast.validate[0] as any).values[0])).toBe(true);
    });
  });
});
