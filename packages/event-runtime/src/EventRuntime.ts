...
export class EventRuntime {
  ...
  private validateBaseTimeRules(
    ...
    if (minDuration) {
      ...
      if (duration !== undefined && duration < minMinutes) {
        errors.push(`持续时间必须至少 ${this.formatDuration(minMinutes)}`);
      }
    }
    ...
    if (maxDuration) {
      ...
      if (duration !== undefined && duration > maxMinutes) {
        errors.push(`持续时间不能超过 ${this.formatDuration(maxMinutes)}`);
      }
    }
    ...
    if (timePrecision) {
      ...
      if (startMinutes % precisionMinutes !== 0) {
        errors.push(`开始时间的分钟数必须是 ${precisionMinutes} 的倍数`);
      }
      const endMinutes = endTime.getUTCMinutes();
      if (endMinutes % precisionMinutes !== 0) {
        errors.push(`结束时间的分钟数必须是 ${precisionMinutes} 的倍数`);
      }
    }
    ...
    if (minAdvanceTime && context.now) {
      ...
      const now =
        context.now instanceof Date ? context.now : new Date(context.now);
      const minStartTime = new Date(now.getTime() + advanceMinutes * 60 * 1000);
      if (startTime < minStartTime) {
        errors.push(
          `开始时间必须至少提前 ${this.formatDuration(advanceMinutes)} 创建`,
        );
      }
    }
    ...
    if (maxAdvanceTime && context.now) {
      ...
      const now =
        context.now instanceof Date ? context.now : new Date(context.now);
      const maxStartTime = new Date(now.getTime() + advanceMinutes * 60 * 1000);
      if (startTime > maxStartTime) {
        errors.push(
          `开始时间不能超过 ${this.formatDuration(advanceMinutes)} 后创建`,
        );
      }
    }
    ...
    if (allowedTimeZones && Array.isArray(allowedTimeZones) && event.timeZone) {
      if (!allowedTimeZones.includes(event.timeZone)) {
        errors.push(`事件时区必须是以下之一: ${allowedTimeZones.join(", ")}`);
      }
    }
  ...
  private evaluateValidationRule(
    ...
    if (rule.type === "Between") {
      ...
      if (isNaN(numValue) || isNaN(numMin) || isNaN(numMax)) {
        return {
          valid: false,
          errors: [`字段 ${rule.field.path.join(".")} 无法进行数值比较`],
        };
      }
      ...
      return {
        ...
        errors: valid
          ? undefined
          : [`字段 ${rule.field.path.join(".")} 必须在 ${min} 和 ${max} 之间`],
      };
    }
    ...
    if (rule.type === "ModComparison") {
      ...
      const result = this.compareValues(modResult, rightValue, rule.operator);
      ...
      return {
        ...
        errors: result
          ? undefined
          : [
              `验证失败: ${rule.left.path.join(".")} mod ${modValue} ${
                rule.operator
              } ${rightValue} 不满足`,
            ],
      };
   ...
   * 比较时间（分钟数）
   */
  private compareTimes(left: number, right: number, operator: string): boolean {
    switch (operator) {
      case "is":
      case "equals":
        return left === right;
