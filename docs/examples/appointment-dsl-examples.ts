...
import { AppointmentType, AppointmentDSL } from '@calenderjs/dsl';
...
export const meetingAppointmentType: AppointmentType = {
  ...
  fields: [
    ...
    },
    {
      ...
      items: {
        type: 'string',
        validation: [
          { type: 'pattern', value: '^[^@]+@[^@]+\\.[^@]+$', errorMessage: '请输入有效的邮箱地址' }
        ]
      }
    },
    {
  ...
  display: {
    ...
    icon: 'meeting',
    titleTemplate: '${title}',
    descriptionTemplate: '${location ? location + " • " : ""}${attendees ? attendees.length + " 人" : ""}'
  },
  behavior: {
    draggable: true,
    resizable: true,
...
export const vacationAppointmentType: AppointmentType = {
  ...
  display: {
    color: '#ff9800',
    icon: 'vacation',
    titleTemplate: '${vacationType === "annual" ? "年假" : vacationType === "sick" ? "病假" : vacationType === "personal" ? "事假" : vacationType === "maternity" ? "产假" : "其他"} - ${title}',
    descriptionTemplate: '${approvalStatus === "approved" ? "✓ 已批准" : approvalStatus === "pending" ? "⏳ 待审批" : "✗ 已拒绝"}${approver ? " by " + approver : ""}'
  },
  behavior: {
...
export const recurringAppointmentType: AppointmentType = {
    ...
    },
    {
      ...
      properties: {
        ...
        interval: {
          ...
          description: '间隔（如每2周）',
          validation: [
            { type: 'min', value: 1, errorMessage: '间隔必须大于0' },
            { type: 'max', value: 100, errorMessage: '间隔不能超过100' }
          ]
        },
        endDate: {
          type: 'date',
        ...
        daysOfWeek: {
          ...
          items: {
            type: 'number',
            validation: [
              { type: 'min', value: 0, errorMessage: '星期值必须在0-6之间' },
              { type: 'max', value: 6, errorMessage: '星期值必须在0-6之间' }
            ]
          }
        }
      }
  ...
  display: {
    ...
    descriptionTemplate: '${recurrence.frequency === "daily" ? "每天" : recurrence.frequency === "weekly" ? "每周" : recurrence.frequency === "monthly" ? "每月" : "每年"}'
  },
  behavior: {
    draggable: true,
    resizable: true,
    ...
    // 全局验证器：检查时间冲突
    {
      ...
      expression: async (appointment: any, allAppointments: any[]) => {
        return !allAppointments.some(apt => {
          ...
          return (apt.startTime < appointment.endTime && apt.endTime > appointment.startTime);
        });
      },
