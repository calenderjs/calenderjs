...
- 2026-01-01: 
  ...
  - 更新架构关系说明：
    - Event Data Model 作为 SSOT（在 `@calenderjs/event-model` 包中）
    - Event DSL 作为生成工具（生成符合 Event Data Model 的数据）
    - 更新依赖关系图
  - 更新 DSL 语法示例（使用 `extra` 字段）
  - 添加时间敏感活动支持章节

**状态**: Draft  
**下一步**: 完成阶段 1.5（时间敏感活动支持）和阶段 1.6（数据模型生成）
